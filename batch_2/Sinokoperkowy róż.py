...
# Korzystaliśmy z materiałów z licencją MIT
# https://github.com/Moduland/pyrgb
# https://stackoverflow.com/questions/59546962/converting-hex-value-to-name-of-the-colour-in-python?fbclid=IwAR1W7bKO6gEsA36bk1eTgfQh1kInCTxq8Aja4SYzlsNQlHZdAT2JzDqFDnI
...
HEIGHT = 250
pygame.init()
pygame.display.set_caption("Color recognition program")
display = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.draw.rect(display, pygame.Color(MAIN_COLOR), pygame.Rect(0, 0, WIDTH, HEIGHT))
pygame.display.flip()
...
while True:
    is_clicked = win32api.GetKeyState(0x01)
    if is_clicked != -127 or is_clicked != -128:
        if win32api.GetKeyState(0x01) == 1:
            win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0)
            win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0)
            is_clicked = 0
    ...
    if is_clicked < 0:
        pygame.draw.rect(display, pygame.Color(color), pygame.Rect(20, 20, 210, 210))
        pygame.display.flip()
        mouse = Controller()
        ...
        for y in range(0, 10, 10):
            for x in range(0, 10, 10):
                pygame.draw.rect(display, pygame.Color(MAIN_COLOR), pygame.Rect(240, 50, 210, 250))
                color = image.getpixel((mouse_pos[0], mouse_pos[1]))
                rgb_val = f'RGB: {color}'
                hex_val = f'Hex: #{pyrgb.rgb_hex(color[0], color[1], color[2])}'
                hsv_val = f'HSV: {pyrgb.rgb_hsv(color[0], color[1], color[2])}'
                hsl_val = f'HSL: {pyrgb.rgb_hsl(color[0], color[1], color[2])}'
                cmyk_val = f'CMYK: {pyrgb.rgb_cmyk(color[0], color[1], color[2])}'
                name = ""
                if pyrgb.rgb_hex(color[0], color[1], color[2]) in colors_dict.keys():
                    name = colors_dict[str(pyrgb.rgb_hex(color[0], color[1], color[2]))]
                else:
                    name = "Color is not defined"

                font = pygame.font.SysFont(None, 20)

                rgb_text = font.render(rgb_val, True, (206, 206, 206))
                display.blit(rgb_text, [250, 50])

                hex_text = font.render(hex_val, True, (206, 206, 206))
                display.blit(hex_text, [250, 80])

                hsv_text = font.render(hsv_val, True, (206, 206, 206))
                display.blit(hsv_text, [250, 110])

                hsl_text = font.render(hsl_val, True, (206, 206, 206))
                display.blit(hsl_text, [250, 140])

                cmyk_text = font.render(cmyk_val, True, (206, 206, 206))
                display.blit(cmyk_text, [250, 170])

                name_text = font.render(name, True, (206, 206, 206))
                display.blit(name_text, [250, 200])
